...
export default function AdminPanel() {
  ...
  const [ram, setRam] = useState<string | undefined>("-")
  ...
  const toggleServer = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data: ServerStatus = await response.json()
        setServerStatus(data)
        setCpu(data.cpuMemory?.cpu)
        setRam(data.cpuMemory?.memory)
        setIsServerOn(true)
  ...
  return (
    <Card className="w-full max-w-md mx-auto border-border bg-neutral-900">
      <CardHeader>
      ...
      <CardContent>
        ...
        {serverStatus && (
          <div className="mt-4 space-y-2 rounded-lg bg-muted p-4 text-sm bg-neutral-900 text-white">
            ...
            {serverStatus.serverIp &&
              <p className="flex justify-between">
                ...
                <span>{serverStatus.serverIp}</span>
              </p>}
          </div>
        )}
      </CardContent>
